...
## Authored
### Purpose
Confirms the inspect symbol neighbors command requests data and surfaces quick pick interactions correctly.

### Notes
- Stubs VS Code window/workspace APIs to validate command registration, empty-editor messaging, and error surface when the server payload is invalid.
...
### Dependencies
- `@copilot-improvement/shared` - `InspectSymbolNeighborsParams` (type-only)
- [`inspectSymbolNeighbors.ParsedInspectSymbolNeighborsResult`](./inspectSymbolNeighbors.ts.md#parsedinspectsymbolneighborsresult) (type-only)
