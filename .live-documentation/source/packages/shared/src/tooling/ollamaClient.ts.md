...
- Generated At: 2025-11-16T02:09:52.129Z

## Authored
### Purpose
Invokes Ollama’s `/api/chat` endpoint with Link-Aware Diagnostics defaults and records structured traces so LLM interactions stay auditable.

### Notes
- Wraps the global `fetch` API with timeout handling, defaulting to a zero-temperature request and injecting a system prompt tailored to relationship extraction unless callers override it.
- Normalises Ollama responses into a minimal `OllamaChatResult`, including optional token counts derived from `eval_count` fields, and raises `OllamaInvocationError` for transport or API failures.
...
## Generated
<!-- LIVE-DOC:PROVENANCE {"generators":[{"tool":"live-docs-generator","version":"0.1.0","generatedAt":"2025-11-16T02:09:52.129Z","inputHash":"3b4c6ee4855e3543"}]} -->
