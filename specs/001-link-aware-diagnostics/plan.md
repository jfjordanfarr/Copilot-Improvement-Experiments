...
Implement the workspace-local docstring bridge pipeline: derive configuration from Layer‑4 MDMD metadata, load the mappings inside the server, and publish diagnostics when docstrings and markdown drift. Build VS Code and CLI tooling to sync docstrings, export bridge status, and visualise change impact as Markdown tables, structured JSON, and ASCII diagrams, aligning with the LLM-readable surface mandate captured in [Layer‑2 Roadmap](../../.mdmd/layer-2/product-roadmap.mdmd.md) and [spec.md](./spec.md#functional-requirements).
...
- Upcoming docstring bridge and narrative commitments will centre on new modules such as `packages/shared/src/tooling/docstringBridgeConfig.ts`, `packages/server/src/features/docbridge/docstringBridgeService.ts`, `packages/server/src/features/diagnostics/publishDocstringDiagnostics.ts`, `packages/shared/src/reporting/rippleNarrative.ts`, and companion tooling in `packages/extension/src/commands/syncDocstrings.ts` plus `scripts/graph-tools/render-ripple.ts`, ensuring Phase 9 delivers workspace-local bridges and LLM-friendly ripple outputs tracked by [`tasks.md`](./tasks.md).
