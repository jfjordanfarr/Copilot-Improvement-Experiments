...
- **Phase 4 – User Story 2 (Developers)**: Extend dependency graph to code modules, reuse official language server data for dependents, enrich diagnostics with quick navigation, and integrate external knowledge-graph feeds in both streaming and on-demand modes with resumable ingestion pipelines that fall back to local inference when remote sources misbehave.
