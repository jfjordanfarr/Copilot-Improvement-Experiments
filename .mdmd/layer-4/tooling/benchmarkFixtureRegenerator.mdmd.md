...
## Purpose
Provide a deterministic CLI that regenerates TypeScript, Python, C, C#, Rust, Java, and Ruby benchmark expectations from language-specific oracles, stages review artefacts, and preserves hand-authored override segments.
- Offer an ergonomic developer workflow (`npm run fixtures:regenerate -- --lang ts|python|c|csharp|rust|java|ruby`) that rebuilds oracle-backed edges without committing changes automatically.
...
### runRegenerationCli
Entry point invoked by the CLI script. Parses suite selection flags, invokes the TypeScript, Python, C, C#, Rust, Java, and Ruby fixture oracles, merges manual override segments, and writes output bundles containing:
...
- [`packages/shared/src/testing/fixtureOracles/csharpFixtureOracle.ts`](../../../packages/shared/src/testing/fixtureOracles/csharpFixtureOracle.ts) walks Roslyn compiler output and Web Forms overrides to derive dependency edges for C# fixtures.
