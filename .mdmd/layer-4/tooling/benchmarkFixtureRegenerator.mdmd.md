...
## Purpose
Provide a deterministic CLI that regenerates TypeScript and Python benchmark expectations from language-specific oracles, stages review artefacts, and preserves hand-authored override segments.
- Offer an ergonomic developer workflow (`npm run fixtures:regenerate -- --lang ts|python`) that rebuilds oracle-backed edges without committing changes automatically.
...
### runRegenerationCli
Entry point invoked by the CLI script. Parses suite selection flags, invokes the TypeScript and Python fixture oracles, merges manual override segments, and writes output bundles containing:
...
## Evidence
- Manual smoke run (`npm run fixtures:regenerate -- --lang python`) verifies `oracle.json`, `merged.json`, and `diff.md` artefacts materialise under `AI-Agent-Workspace/tmp/fixture-regeneration/<fixture-id>/`.
