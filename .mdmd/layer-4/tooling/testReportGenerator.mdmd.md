...
## Purpose
Produce durable per-mode artifacts (`reports/test-report.self-similarity.md`, `reports/test-report.ast.md`, etc.) that capture the outcome of regression, benchmark, and telemetry runs so stakeholders can audit accuracy and performance trends without re-running the suites.
